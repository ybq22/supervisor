...
### Step 4: Preview and Confirm / 预览与选项确认

Show the user a summary, ask for confirmation, and ASK for their preferred installation method:
```
...
1. Install using .agents specification (Recommended for multi-CLI compatibility)
2. Install directly to a specific CLI (e.g., .claude)

If 1, which CLIs would you like to link to? (e.g., claude, gemini)
If 2, which target CLI?
```

### Step 5: Skill Generation & Installation / 生成与安装

Use `${CLAUDE_SKILL_DIR}/prompts/builder.md` to generate the content, then use the `Write` tool (or bash commands) to save the files based on the user's choice.

**Option 1: .agents specification**
1. Write Profile JSON to `~/.agents/mentors/{name}.json`
2. Write Mentor Skill to `~/.agents/skills/{name}/SKILL.md`
3. Update `~/.agents/.skill-lock.json` to register the new skill path.
4. For each selected CLI (e.g., `claude`, `gemini`), use `Bash` to create symlinks:
   ```bash
   ln -s ../../.agents/mentors/{name}.json ~/.{cli}/mentors/{name}.json
   ln -s ../../.agents/skills/{name} ~/.{cli}/skills/{name}
   ```
   *(On Windows, fallback to copying files if symlinks fail due to permissions).*

**Option 2: Traditional installation**
1. Write Profile JSON to `~/.{cli}/mentors/{name}.json`
2. Write Mentor Skill to `~/.{cli}/skills/{name}/SKILL.md`
...
| Read existing profiles | `Read` tool on existing JSON profiles |
| Write generated files | `Write` tool |
| Generate mentor skill | Use prompts in `${CLAUDE_SKILL_DIR}/prompts/` |
| Create compatibility links | `Bash` tool (ln -s or cp) |

## 输出位置 / Output Locations

Depends on user choice:
- **.agents**: `~/.agents/mentors/` and `~/.agents/skills/` (with links to target CLIs)
- **Traditional**: `~/.{cli}/mentors/` and `~/.{cli}/skills/`
